...
from microninja_apps.Media import media_dir
...
def _create_kdesk_icon(app):
    ...
    #checking hover icon name
    title = app["title"]
    title = title.replace(".", "")
    truncate = title.find("(")

    if truncate > 0:
        title = title[:(truncate-1)]
    title = title.replace(" ", "-")  
    title += "-hover.png"      
    
    hover_icon_name = media_dir() + "icons/" + title
    icon_hover = icon[:-4] + "-hover.png"
    if os.path.exists(icon_hover):
        kdesk_entry += '  IconHover: {}\n'.format(icon_hover)
    elif os.path.exists(hover_icon_name):
        kdesk_entry += '  IconHover: {}\n'.format(hover_icon_name)
    else:
        kdesk_entry += '  IconHover: {}\n'.format(media_dir() +
