...
from microninja_apps.AppManage import uninstall_packages, KDESK_EXEC, \
    uninstall_link_and_icon
from microninja_apps.AppData import load_from_app_file
from microninja_apps.DesktopManage import add_to_desktop, remove_from_desktop
from microninja_apps.Media import media_dir, get_app_icon
...
class Apps(Gtk.Notebook):
    def __init__(self, apps, main_win):
        ...
        context.add_class("notebook-tab")
        want_more_app = {
            "type": "app",
            "title": "Quali altre applicazioni?",
            "tagline": "Vai su Microninja e installane altre",
            "slug": "want-more",
            ...
            "categories": ["system", "internet", "code", "games",
                           "tools"],

            "packages": [],
            "dependencies": ["chromium"],
            "launch_command": {"cmd": "kdesk-blur",
                               "args": ["microninja-world-launcher /apps/"]},
            "overrides": [],
            "desktop": False
        }
        ...
        #this must be the same name in the left menu
        system_menu = _("system")
        #this must be the same name in the left menu
        internet_menu = _("internet")
        #this must be the same name in the left menu
        coding_menu = _("coding")
        #this must be the same name in the left menu
        games_menu = _("games")
        #this must be the same name in the left menu
        tools_menu = _("tools")

        self._cat_names = [system_menu, internet_menu, coding_menu, games_menu, tools_menu]
        self._categories = {}

        self._apps = {}
        self.evtBox = []
        # Determine which categories have apps (the others won't be shown)
        #used_categories = set(["ultime", "altre"])
        used_categories = set([])
        #for app in apps:
        #    if 'categories' in app:
        #        for category in app['categories']:
        #            if category in want_more_app["categories"]:
        ...
        for index , catName in enumerate(self._cat_names):
            cat = want_more_app["categories"][index]
            self._categories[cat] = AppGrid(main_win, self)
            label = Gtk.Label(catName.upper())
    ...
    def add_app(self, app_data):
        ...
        if app_data["type"] == "app":
            if "categories" in app_data:
                ...
                # inside others
                #if not shown:
                #    cat_obj = self._categories["altre"]
                #    entry = cat_obj.new_entry(app_data)
                #    app_entries.append(entry)
        #elif app_data["type"] == "dentry":
        #    entry = self._categories["altre"].new_entry(app_data)
        #    app_entries.append(entry)

        #if "time_installed" in app_data:
        #    entry = self._categories["ultime"].new_entry(
...
class AppGrid(Gtk.EventBox):
    ...
    def new_entry(self, app, sort_by="title", reverse=False, index = 0):
        ...
        entry = AppGridEntry(app, self._win, self._apps, self.tot)
        self.tot += 1
...
class AppGridEntry(Gtk.EventBox):
    def __init__(self, app, window, apps_obj,index):
        ...
        context = self.get_style_context()
        if index % 2 != 0 :
            context.add_class("app-grid-odd")
        context.add_class("colorBox")
        self._app = app
    ...
    def _setup_desc_button(self):
        ...
        more_btn.set_tooltip_text("Altre informationi")
    ...
    def _setup_desktop_button(self):
        dir_a = "/usr/share/microninja-desktop/kdesk/kdesktop/"
        for f in os.listdir(dir_a):
			...
			for line in link.split("\n"):
				...
				if l.startswith("command:"):
					...
					if exe == self._app["launch_command"]["cmd"] and len(self._app["launch_command"]["args"]) == 0 :
						return True


        self._desktop_btn = None
        if "_install" not in self._app and \
           ("desktop" not in self._app or self._app["desktop"]) and self._app["_default"]==False:
    ...
    def _uninstall_cb(self, event):
        confirmation = KanoDialog(
            title_text=_("Remove {}").format(self._app["title"]),
            description_text=_("This application will be uninstalled and "
                             "removed from the list. Do you want to proceed?"),
            button_dict={
                _("YES"): {
                    "return_value": 0
                },
        ...
        prompt = _("Uninstall {}").format(self._app["title"])
    ...
    def _update_cb(self, widget):
        confirmation = KanoDialog(
            title_text=_("Update {}").format(self._app["title"]),
            description_text=_("This application will be updated. Do you want to proceed?"),
            button_dict={
                _("YES"): {
                    "return_value": 0
                },
